...
tag @a[scores={HP=..5,job=3},tag=!fresh,tag=!10_skill_cding] add 3_skill_10_start
    scoreboard players set @a[scores={HP=..5,job=3},tag=!fresh] cd_10 40
...
execute as @a[scores={job=3},tag=!10_skill_cding] unless entity @s[nbt={Inventory:[{Slot:9b,id:"minecraft:glow_ink_sac",Count:1b}]}] run clear @s minecraft:glow_ink_sac
    execute as @a[scores={job=3},tag=!10_skill_cding] unless entity @s[nbt={Inventory:[{Slot:9b,id:"minecraft:glow_ink_sac",Count:1b}]}] run item replace entity @s inventory.0 from block -4961 71 -5058 inventory.0

#循环
schedule function ci:loop/game/job/3_maw/skill/p 1t append
