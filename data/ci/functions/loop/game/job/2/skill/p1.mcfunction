...
execute as @a[scores={2_skill_10_test=80..,job=2},tag=!fresh,team=war] unless entity @s[scores={death_test=1..}] run tag @s add 2_skill_10_start
    execute as @a[scores={2_skill_10_test=80..,job=2},tag=!fresh,team=war] unless entity @s[scores={death_test=1..}] run scoreboard players set @s cd_10 600
...
#循环
schedule function ci:loop/game/job/2/skill/p1 1t append
