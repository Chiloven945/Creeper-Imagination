scoreboard players reset $tool choose_job_time
scoreboard players set $tool game_stage 3
execute as @a at @s run playsound minecraft:item.goat_horn.sound.0 player @s ~ ~ ~ 100 0.9
tag @a[scores={team=1..2}] add ready_for_war
function steal:module/choose_job/break_barrier
bossbar set ci:choose_job players
