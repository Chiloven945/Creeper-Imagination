execute at @s run playsound minecraft:block.dispenser.dispense player @a[distance=..15] ~ ~ ~ 100 1
execute at @s run playsound minecraft:entity.ender_eye.launch player @a[distance=..15] ~ ~ ~ 100 0

execute as @s at @s anchored eyes positioned ^ ^ ^-1 run summon spectral_arrow ~ ~ ~ {Tags:["job_2_1_new","job_2_1"],ShotFromCrossbow:1b,LeftOwner:1b,shake:0b,crit:0b,pickup:0b,damage:1.0,SoundEvent:"minecraft:entity.player.attack.crit"}
